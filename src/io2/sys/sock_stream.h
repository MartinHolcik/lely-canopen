/**@file
 * This is the internal header file of the stream socket declarations.
 *
...
#ifndef LELY_IO2_INTERN_SYS_SOCK_STREAM_H_
#define LELY_IO2_INTERN_SYS_SOCK_STREAM_H_
 ...
 * associated attributes for a stream socket.
 */
struct io_sock_stream_handle {
...
/// The static initializer for #io_sock_stream_handle.
#if _WIN32
#define IO_SOCK_STREAM_HANDLE_INIT \
	{ \
...
/// The implementation of a stream socket.
struct io_sock_stream_impl {
	...
	/// A pointer to the virtual table for the stream socket interface.
	const struct io_sock_stream_vtbl *sock_stream_vptr;
	/**
	...
	/// The task responsible for intiating connect operations.
	struct ev_task connect_task;
	/**
	...
	/// A flag indicating whether #connect_task has been posted to #exec.
	unsigned connect_posted : 1;
...
#endif
	/// The queue containing pending connect operations.
	struct sllist connect_queue;
	/// The connect operation currently being executed.
	struct ev_task *current_connect;
#if _WIN32
	/**
	 * The queue containing successfully initiated connect operations
	 * waiting for a completion packet.
	 */
	struct sllist connect_iocp_queue;
#endif
	/**
...
int io_sock_stream_impl_init(struct io_sock_stream_impl *impl, io_poll_t *poll,
		ev_exec_t *exec, const struct io_endp_vtbl *endp_vptr);
void io_sock_stream_impl_fini(struct io_sock_stream_impl *impl);

void io_sock_stream_impl_get_handle(const struct io_sock_stream_impl *impl,
		struct io_sock_stream_handle *phandle);
SOCKET io_sock_stream_impl_open(
		struct io_sock_stream_impl *impl, int family, int protocol);
int io_sock_stream_impl_assign(struct io_sock_stream_impl *impl,
		const struct io_sock_stream_handle *handle);
SOCKET io_sock_stream_impl_release(struct io_sock_stream_impl *impl);
...
#endif // !LELY_IO2_INTERN_SYS_SOCK_STREAM_H_
